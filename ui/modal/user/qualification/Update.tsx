...
const UpdateQualification = forwardRef<UpdateQualificationRef, UpdateQualificationProps>(
    ({ updateQualification }, ref) => {
        const [show, setShow, setHide] = useBoolean(false);
        const qualificationRef = useRef<Education>();
        const [updateEducation, { isSuccess, data: education }] = useUpdateEducationMutation();
        useImperativeHandle(
            ref,
            () => ({
