...
import DeleteQualificationModal from "../../../ui/modal/user/qualification/Delete";
import UpdateQualification from "../../../ui/modal/user/qualification/Update";
...
const UserQualification = ({ educations, isMyPage, ...rest }: UserQualificationProps & SxProps) => {
    const [eds, setEds] = useState(educations);
    const { removingItem, showConfirm, setHideConfirm, onConfirmDelete, openRemoveModal } =
        useRemoveQualification(setEds);
    const { updateRef, openUpdateModal, onUpdate } = useUpdateQualification(setEds);
        ...
        <Card sx={{ ...rest, p: 1 }}>
            ...
            <Gallery height={250} spacing={2} pxPerClick={366}>
                <>
                    ...
                    {isMyPage && <UpdateQualification ref={updateRef} updateQualification={onUpdate} />}
                </>
            </Gallery>
        </Card>
    );
