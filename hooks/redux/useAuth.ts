...
const loginFromResponse = (
  trigger: (opt: any, cache: boolean) => void,
...
export const useLoginFromCreds = (): [
...
export const useLoginFromVerification = (): [
  (token: string) => void,
  RTKStatus
] => {
  const [trigger] = useLazyGetSelfQuery();
...
export const useRestorePassword = (): [
  (restoreCredentials: UserCredentials) => void,
  RTKStatus
] => {
  const [trigger] = useLazyGetSelfQuery();
  const { enqueueSnackbar } = useSnackbar();
...
export const useForgetPassword = (): [
  (restoreMail: UserCredentials) => void,
  RTKStatus
] => {
  const { enqueueSnackbar } = useSnackbar();
