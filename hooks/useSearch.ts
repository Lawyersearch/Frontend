...
import {useLazyGetUsersByIdQuery} from "../services/search";
...
export const useSearch = (view: CategoryView[]) => {
  ...
  const categoryRef = useRef<{selected: string, expanded: string[]}>({ selected: '0', expanded: [] })
  const [trigger, { data: users, isLoading, isSuccess, error }] = useLazyGetUsersByIdQuery()
  useEffect(() => {
